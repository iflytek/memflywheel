...
Zero runtime dependencies — Node stdlib + TypeScript only. The core never calls an LLM; extraction and dream consolidation are supplied by the host through the `ExtractionAgentRunner` and `DreamAgentRunner` contracts — both tool-calling subagents that write memory files via core's memory tools.
