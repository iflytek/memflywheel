...
The MemScribe memory kernel: file-backed Markdown + YAML-frontmatter storage, derived `MEMORY.md` index, progressive index recall, extraction and dream consolidation with pluggable model injection points, privacy redaction, per-root write locking, atomic writes, and an append-only audit log.
