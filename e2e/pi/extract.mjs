/**
 * Standalone memflywheel extraction script for Pi K8s E2E test.
 *
 * Pi's --print mode exits immediately after responding, so the extension-based
 * async extraction doesn't complete. This script provides a standalone
 * extraction step that:
 *   1. Reads Pi's session file to get the conversation
 *   2. Sends it to mock-llm with file tools (the extraction agent flow)
 *   3. Executes tool calls (glob, write) to create memory files
 *
 * Usage: node /e2e/pi/extract.mjs <session-id> <workspace-dir>
 */
...
async function chat(messages, toolResults) {
  const body = {
    model: "mock-llm",
    stream: false,
    messages,
...
// Each prompt is extracted separately (matching Hermes behavior where each
// turn triggers its own extraction)
const prompts = process.argv.slice(3);
if (prompts.length === 0) {
  // Default prompts matching the e2e test
  prompts.push(
...
async function buildMemoryIndex(workspaceDir) {
  ...
  for (const f of files) {
    ...
    // Include body content (after frontmatter) so keyword assertions can match
