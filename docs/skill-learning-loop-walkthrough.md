...
```
宿主在 turn-end 把这一轮对话轨迹交给 MemFlywheel
  → 先做记忆增量提取
  → 提取成功且门控通过，才跑技能演化
...
MemFlywheel **从不执行技能**；它只 store / validate / evolve / recall。执行永远是宿主主 Agent 的事。
...
```
 ...
 宿主对话 ──┼─► turn-end ─► 记忆提取 ─► 技能演化 ─► dream(压缩记忆成 cue)                   │
            │                  │            │            │                                  │
            │                  ▼            ▼            ▼                                  │
            │           memory/*.md   skills/memflywheel-learned-*/   MEMORY.md(索引)        │
            └──────────────────────────────────────────────────────────────────────────────┘
...
skillsRoot/                         ← 已发布的 learned skills（对外、可被召回/执行）
  memflywheel-learned-foo/SKILL.md
...
非 noop 时（这就是记忆↔技能联动的来源）：
   ...
   memoryTopics = [ humanizeSkillSlug(targetSkill) ]
                  └─ "memflywheel-learned-monorepo-release" → "monorepo release"
...
MemFlywheel 不执行、不包装主 Agent 的读/执行链路
```
...
模型工具写入限制在 stage 沙箱；finalize 由系统发布到 skillsRoot；失败一律回滚到发布前快照；MemFlywheel 永不执行技能。
...
```
adapters/src/host-memflywheel.ts      createMemFlywheelHarnessRuntime / assembledSkillEvolution / fail-fast
