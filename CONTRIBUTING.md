...
Thanks for helping improve MemScribe.

MemScribe is a file-native long-term memory layer for AI agents. It is a memory
...
Please keep changes inside the public MemScribe scope:
...
| LLM calls | `@memscribe/core` must not call LLMs directly. Use injected runners or `toolCompletion`. |
...
1. Run `pnpm run ci`.
2. Check that package metadata points to `iflytek/memscribe`.
